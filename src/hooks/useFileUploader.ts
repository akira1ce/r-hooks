	...
	const checkFileStatus = async (hash: string, filename: string) => {
		try {
			const response = await fetch(
				`/api/upload?hash=${hash}&filename=${filename}`,
			);
			...
			if (data.exists) {
				...
				setFiles((prev) =>
					prev.map((file) =>
						file.raw.name === filename ? { ...file, url: data.url } : file,
					),
				);
	...
	const uploadFile = async (file: UploadFile) => {
		try {
			...
			if (!hash) {
				...
				setFiles((prev) =>
					prev.map((f) => (f.raw.name === raw.name ? { ...f, hash } : f)),
				);
			}

			// Check if file already exists or get uploaded chunks
			updateFileProgress(raw.name, { message: "检查文件状态..." });
			const { exists, uploadedChunks = [] } = await checkFileStatus(
				hash,
				raw.name,
			);
			...
			for (let i = 0; i < totalChunks; i++) {
				...
				if (result.url) {
					...
					setFiles((prev) =>
						prev.map((f) =>
							f.raw.name === raw.name ? { ...f, url: result.url } : f,
						),
					);
